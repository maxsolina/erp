...
// Extraído del monolito (SeccionSimulador). Self-contained.

import { useEffect, useState } from "react"
import { AlertCircle, Calculator, Percent } from "lucide-react"
import { CUOTAS_OPTIONS, SectionHeader, formatCurrency, formatPct, type Tarjeta, type GrupoTarjeta, type RecargoTarjeta } from "./_shared"
...
export default function Simulador() {
  ...
  const calcular = () => {
    ...
    const desglose = (grupo?.cargos || []).map(c => ({ nombre: c.nombre, pct: c.arancel, importe: montoNum * (c.arancel / 100) }))
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <div className="bg-white rounded-lg shadow-sm p-6 space-y-4">
          ...
          <div>
            ...
            <select value={tarjetaId} onChange={e => setTarjetaId(e.target.value === "" ? "" : parseInt(e.target.value))} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none">
          ...
          <div>
            ...
            <div className="flex gap-2 flex-wrap">
              {CUOTAS_OPTIONS.map(c => (
                <button key={c} onClick={() => setCuotas(c)} className={`px-3 py-1.5 text-sm rounded border transition-colors ${cuotas === c ? "bg-emerald-600 text-white border-emerald-600" : "bg-white text-gray-700 border-gray-300 hover:border-emerald-400"}`}>{c}c</button>
              ))}
            </div>
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Monto (ARS)</label>
            <input type="text" placeholder="Ej: 50000" value={monto} onChange={e => setMonto(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none" />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Fecha de pago</label>
            <input type="date" value={fecha} onChange={e => setFecha(e.target.value)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-emerald-500 focus:outline-none" />
          </div>
          <button onClick={calcular} disabled={!tarjetaId || !monto} className="w-full py-2 bg-emerald-700 text-white rounded-md font-medium hover:bg-emerald-800 disabled:bg-gray-300 disabled:cursor-not-allowed">Calcular recargo</button>
        </div>
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          {noEncontrado && (
            <div className="flex items-center gap-3 bg-amber-50 border border-amber-200 rounded-lg p-4 text-amber-700">
              ...
              <p className="text-sm">No se encontró un recargo configurado para la combinación seleccionada.</p>
            </div>
          )}
          {resultado && (
            <div className="space-y-4">
              <div className="bg-gray-50 rounded-lg p-3 text-sm space-y-1">
                <div className="flex justify-between text-gray-600"><span>Tarjeta:</span><span className="font-medium">{tarjetas.find(t => t.id === tarjetaId)?.nombre} — {cuotas} cuota{cuotas > 1 ? "s" : ""}</span></div>
                <div className="flex justify-between text-gray-600"><span>Grupo:</span><span className="font-medium">{resultado.grupo?.nombre}</span></div>
                <div className="flex justify-between text-gray-600"><span>Monto base:</span><span className="font-medium">{formatCurrency(montoNum)}</span></div>
              </div>
