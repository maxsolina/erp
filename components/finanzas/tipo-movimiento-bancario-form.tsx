...
const BOOL_FIELDS = [
  { key: "emite_cheques_diferidos", label: "Emite Cheques Diferidos" },
  { key: "emite_cheques_corrientes", label: "Emite Cheques Corrientes" },
  { key: "disponible_en_pagos", label: "Disponible en Pagos" },
  { key: "disponible_en_cobros", label: "Disponible en Cobros" },
  { key: "disponible_en_finanzas", label: "Disponible en Finanzas" },
] as const
...
const empty: Form = {
  nombre: "",
  codigo_causal: "",
  emite_cheques_diferidos: false,
  emite_cheques_corrientes: false,
  disponible_en_pagos: false,
  disponible_en_cobros: false,
  disponible_en_finanzas: true,
  activo: true,
}

export default function TipoMovimientoBancarioForm({ initialId }: { initialId?: string }) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/tipos-movimiento-bancario/${initialId}`)
      .then(r => r.ok ? r.json() : Promise.reject(r))
      .then((d: Partial<Form>) => { setForm({ ...empty, ...d }); setCargando(false) })
      .catch(() => { setErrorCarga("Tipo de movimiento no encontrado"); setCargando(false) })
  }, [isEdit, initialId])
  ...
  const guardar = async () => {
    ...
    if (!form.codigo_causal.trim()) return setError("El código causal es obligatorio")
    if (guardando) return
    setError(null)
    setGuardando(true)
    try {
      const res = await fetch(
        isEdit ? `/api/tipos-movimiento-bancario/${initialId}` : "/api/tipos-movimiento-bancario",
    ...
    <div className="max-w-2xl">
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold text-amber-900">{isEdit ? "Editar Tipo de Movimiento" : "Nuevo Tipo de Movimiento"}</h1>
        </div>
      ...
      <div className="bg-white rounded-lg border p-6 space-y-5">
        <div className="grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-gray-600 mb-1">Código Causal *</label>
            <input value={form.codigo_causal} onChange={e => set("codigo_causal", e.target.value)}
              className="w-full border rounded px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              placeholder="ej: TR, CHQD" />
          </div>
        </div>
